...
final _formatted = RegExp(
  r'^BK-[0-9A-HJKMNP-TV-Z]{4}(-[0-9A-HJKMNP-TV-Z]{4}){5}-[0-9A-HJKMNP-TV-Z]{3}$',
);
...
void main() {
  test('carries 128 bits from the secure generator, all distinct', () {
    final codes = List.generate(
      1000,
      (_) => SecretCode.generate(SecretCodeKind.backupRecovery),
    );
  ...
  test('parses back exactly, tolerating how people type it', () {
    ...
    for (final v in variants) {
      expect(
        SecretCode.parse(v, SecretCodeKind.backupRecovery).bytes,
        code.bytes,
        reason: 'variant $v',
      );
    }
  ...
  test('a code with any other prefix is rejected as not a Backup Recovery '
      'Key', () {
    final code = SecretCode.generate(SecretCodeKind.backupRecovery);
    for (final prefix in ['AR', 'XX']) {
      expect(
        _problem('$prefix-${code.formatted.substring(3)}',
            SecretCodeKind.backupRecovery),
  ...
  test('single-character typos are caught by the check character '
    ...
    for (var i = 0; i < trials; i++) {
      ...
      final problem = _problem(chars.join(), SecretCodeKind.backupRecovery);
