...
| **Schema amendment 14 (schema v2)**: `plan_imports.imported_by` becomes nullable while there is no authentication — the importer stays NULL, never a placeholder user (docs/04 §9 item 14). Restore accepts schema versions 1–2 | **USER-DECIDED**, 2026-09-24 (Phase 1.6, Option B) |
| **Micro Plan import matching**: same name + same code = one institution; anything similar is a yes/no suggestion for the MO, never merged automatically; PS/UPS with the same name are different schools; rows without a code get their own flagged record; school code is kept but is not identity (a code already in use is left blank with a note); all stored text in CAPITAL letters (docs/MICRO_PLAN_FILE_FORMAT_SPEC.md) | **USER-DECIDED**, 2026-09-25 (Phase 1.6) |
| **Micro Plan re-import (option A)**: one live import per financial year — a second one is refused. An import can be undone only while no work has started on any of its visits/holidays; undo soft-deletes its visits, holidays and the schools/AWCs it created (a removed school's code is released). Field work is never erased | **USER-DECIDED**, 2026-09-25 (Phase 1.6) |
...
| | |
|---|---|
| Frozen schema version | v1.0 (Phase 0.6) + amendment 14 (Phase 1.6: `plan_imports.imported_by` nullable), implemented as Drift `schemaVersion = 2` |
...
| Migration status | Version 1 = the full frozen schema in one `onCreate` pass. Version 2 (Phase 1.6) = one `onUpgrade` step rebuilding `plan_imports` with `imported_by` nullable, every row copied (docs/04 §9 item 14). Never destructive recreation. |
