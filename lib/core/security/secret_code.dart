...
/// Which recovery secret a code is. The prefix is shown to people so the two
/// codes are never confused; it is not part of the secret.
enum SecretCodeKind {
  /// Resets the Admin PIN on this phone (docs/30 R1).
  adminRecovery('AR'),
...
/// the real code.
enum SecretCodeProblem { invalidFormat, wrongKind, checkFailed }
...
/// character, in groups of four — e.g. `AR-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXX`.
///
/// The check character is 5 bits of SHA-256 over (kind, secret). It catches
/// most typing mistakes before they count as a failed attempt, and makes an
/// Admin Recovery Code typed where a Backup Recovery Key is expected (or the
/// reverse) fail clearly. It reveals nothing useful about the secret.
class SecretCode {
  ...
  factory SecretCode.parse(String input, SecretCodeKind kind) {
    ...
    if (text.length == totalChars + 2) {
      ...
      } else if (SecretCodeKind.values.any((k) => k.prefix == prefix)) {
        throw const SecretCodeFormatException(SecretCodeProblem.wrongKind);
      } else {
    ...
    if (text[_dataChars] != _checkChar(kind, bytes)) {
      // The same bytes with the other kind's check means the wrong code type.
      final other = SecretCodeKind.values.firstWhere((k) => k != kind);
      throw SecretCodeFormatException(
        text[_dataChars] == _checkChar(other, bytes)
